...
import "highlight.js/styles/github.css"; // You can switch theme here
          ...
          components={{
            code({ node, className, children, ...props }) {
              ...
              return !isInline ? (
                ...
                <code className="px-1 py-0.5 bg-gray-100 text-sm rounded" {...props}>
                  {children}
                </code>
              );
            },
            // Other markdown component overrides can stay the same
...
// ✅ CodeBlock: supports copy + syntax highlighting
function CodeBlock({ code, language, node }) {
  ...
  return (
    <div className="relative my-4 rounded-md bg-gray-900 text-white overflow-hidden">
      {/* Header */}
      <div className="flex justify-between items-center px-4 py-2 bg-gray-800 border-b border-gray-700 text-sm">
        <div className="flex items-center gap-2">
          <LuCode className="text-cyan-300" size={16} />
          <span className="uppercase text-cyan-200 font-semibold text-xs">
            {language || "Code"}
          </span>
        </div>
        <button
          onClick={copyCode}
          className="flex items-center gap-1 text-xs text-white bg-gray-700 px-2 py-1 rounded hover:bg-gray-600 transition-all"
